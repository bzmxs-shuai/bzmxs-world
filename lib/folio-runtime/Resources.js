...
{
    constructor(assetBasePath = '/folio')
    {
        super()
        ...
        this.loader.load([
            ...
            ...resource,
            source: resource.source.startsWith('./') ? `${this.assetBasePath}/${resource.source.slice(2)}` : resource.source
        })))
        ...
        this.loader.on('fileError', (_resource, _failure) =>
        {
            this.failures.push(_failure)
            this.trigger(_failure.required ? 'error' : 'warning', [_failure])
            this.trigger('progress', [this.loader.loaded / this.loader.toLoad])
        })

        this.loader.on('fatal', (_failure) =>
        {
            this.fatalFailure = _failure
            this.trigger('fatal', [_failure])
        })
