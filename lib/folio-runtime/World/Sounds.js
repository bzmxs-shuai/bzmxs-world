...
export default class Sounds
{
    ...
    setSettings()
    {
        ...
        {
            _settings.sounds = _settings.sounds.map((_sound) => this.getAssetPath(_sound))
            this.add(_settings)
        }
    }
    ...
    getAssetPath(_path)
    {
        return _path.startsWith('./') ? `${this.assetBasePath}/${_path.slice(2)}` : _path
    }
    ...
    setEngine()
    {
        ...
        this.engine.sound = new Howl({
            src: [this.getAssetPath('./sounds/engines/1/low_off.mp3')],
            loop: true
        })
